...
import type { SlottingSchema } from '../../rayfin/data/schema';
...
let client: RayfinClient<SlottingSchema> | null = null;
let localDev = false;

export function initRayfinClient(
  config: RayfinClientConfig
): RayfinClient<SlottingSchema> {
  if (client) {
    throw new Error('Rayfin client is already initialized.');
  }
  client = new RayfinClient<SlottingSchema>({
...
export function getRayfinClient(): RayfinClient<SlottingSchema> {
  if (!client) {
    throw new Error(
